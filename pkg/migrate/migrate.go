...
func Run(opts Options) (*Result, error) {
	...
	fasterClawHome, err := resolveFasterClawHome(opts.FasterClawHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, fasterClawHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to FasterClaw")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, fasterClawHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, fasterClawHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(fasterClawHome, "config.json"),
				Description: "convert OpenClaw config to FasterClaw format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(fasterClawHome)
...
func Execute(actions []Action, openclawHome, fasterClawHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, fasterClawHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, fasterClawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing FasterClaw config: %w", err)
		}
...
func resolveFasterClawHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("FASTERCLAW_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".fasterclaw"), nil
}
