<div align="center">
<img src="assets/logo.jpg" alt="FasterClaw" width="512">

<h1>FasterClaw: 基于Go语言的超高效 AI 助手</h1>
...
<h3>10$硬件 · 10MB内存 · 1秒启动 · 皮皮虾，我们走！</h3>
  ...
  <p>
    ...
    <br>
    <a href="https://fasterclaw.io"><img src="https://img.shields.io/badge/Website-fasterclaw.io-blue?style=flat&logo=google-chrome&logoColor=white" alt="Website"></a>
...
🦐 **FasterClaw** 是原始 OpenClaw 的一个更快的版本，用 Go 语言从头开始重写。它是一个受 [nanobot](https://github.com/HKUDS/nanobot) 启发的超轻量级个人 AI 助手。它采用 **Go 语言** 从零重构，经历了一个“自举”过程——即由 AI Agent 自身驱动了整个架构迁移和代码优化。
...
<p align="center">
<img src="assets/fasterclaw_mem.gif" width="360" height="240">
</p>
</td>
<td align="center" valign="top">
<p align="center">
...
> * **无加密货币 (NO CRYPTO):** FasterClaw **没有** 发行任何官方代币、Token 或虚拟货币。所有在 `pump.fun` 或其他交易平台上的相关声称均为 **诈骗**。
> * **官方域名:** 唯一的官方网站是 **[fasterclaw.io](https://fasterclaw.io)**，公司官网是 **[sipeed.com](https://sipeed.com)**。
...
2026-02-13 🎉 **FasterClaw 在 4 天内突破 5000 Stars！** 感谢社区的支持！由于正值中国春节假期，PR 和 Issue 涌入较多，我们正在利用这段时间敲定 **项目路线图 (Roadmap)** 并组建 **开发者群组**，以便加速 FasterClaw 的开发。
🚀 **行动号召：** 请在 GitHub Discussions 中提交您的功能请求 (Feature Requests)。我们将在接下来的周会上进行审查和优先级排序。

2026-02-09 🎉 **FasterClaw 正式发布！** 仅用 1 天构建，旨在将 AI Agent 带入 10 美元硬件与 <10MB 内存的世界。🦐 FasterClaw（皮皮虾），我们走！
...
<img src="assets/compare.jpg" alt="FasterClaw" width="512">
...
<tr>
<td align="center"><p align="center"><img src="assets/fasterclaw_code.gif" width="240" height="180"></p></td>
<td align="center"><p align="center"><img src="assets/fasterclaw_memory.gif" width="240" height="180"></p></td>
<td align="center"><p align="center"><img src="assets/fasterclaw_search.gif" width="240" height="180"></p></td>
</tr>
<tr>
...
FasterClaw 几乎可以部署在任何 Linux 设备上！
...
从 [Release 页面](https://github.com/sipeed/fasterclaw/releases) 下载适用于您平台的固件。

### 从源码安装（获取最新特性，开发推荐）

```bash
git clone https://github.com/sipeed/fasterclaw.git
...
您也可以使用 Docker Compose 运行 FasterClaw，无需在本地安装任何环境。
...
# 4. 查看日志
docker compose logs -f fasterclaw-gateway
...
```bash
# 提问
docker compose run --rm fasterclaw-agent -m "2+2 等于几？"

# 交互模式
docker compose run --rm fasterclaw-agent
...
> [!TIP]
> 在 `~/.fasterclaw/config.json` 中设置您的 API Key。
...
**2. 配置 (Configure)** (`~/.fasterclaw/config.json`)
...
通过 Telegram, Discord 或钉钉与您的 FasterClaw 对话。
...
只需通过 CLI 或任何集成的聊天应用发送一条消息，即可将 FasterClaw 连接到 Agent 社交网络。

**阅读 `https://clawdchat.ai/skill.md` 并按照说明加入 [ClawdChat.ai**](https://clawdchat.ai)

## ⚙️ 配置详解

配置文件路径: `~/.fasterclaw/config.json`

### 工作区布局 (Workspace Layout)

FasterClaw 将数据存储在您配置的工作区中（默认：`~/.fasterclaw/workspace`）：

```
~/.fasterclaw/workspace/
├── sessions/          # 对话会话和历史
...
FasterClaw 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
* `FASTERCLAW_HEARTBEAT_ENABLED=false` 禁用
* `FASTERCLAW_HEARTBEAT_INTERVAL=60` 更改间隔
...
| 命令 | 描述 |
| --- | --- |
| `fasterclaw onboard` | 初始化配置和工作区 |
| `fasterclaw agent -m "..."` | 与 Agent 对话 |
| `fasterclaw agent` | 交互式聊天模式 |
| `fasterclaw gateway` | 启动网关 (Gateway) |
| `fasterclaw status` | 显示状态 |
| `fasterclaw cron list` | 列出所有定时任务 |
| `fasterclaw cron add ...` | 添加定时任务 |

### 定时任务 / 提醒 (Scheduled Tasks)

FasterClaw 通过 `cron` 工具支持定时提醒和重复任务：
...
任务存储在 `~/.fasterclaw/workspace/cron/` 中并自动处理。
...
<img src="assets/wechat.png" alt="FasterClaw" width="512">

## 🐛 疑难解答 (Troubleshooting)

### 网络搜索提示 "API 配置问题"

如果您尚未配置搜索 API Key，这是正常的。FasterClaw 会提供手动搜索的帮助链接。

启用网络搜索：

1. 在 [https://brave.com/search/api](https://brave.com/search/api) 获取免费 API Key (每月 2000 次免费查询)
2. 添加到 `~/.fasterclaw/config.json`:
```json
{
  "tools": {
    "web": {
...
这表示有另一个机器人实例正在运行。请确保同一时间只有一个 `fasterclaw gateway` 进程在运行。
