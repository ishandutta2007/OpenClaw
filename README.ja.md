...
<h1>FasterClaw: Go で書かれた超効率 AI アシスタント</h1>
...
🦐 FasterClaw は、オリジナルのOpenClawをGo言語でゼロから書き直した、より高速なバージョンです。[nanobot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。Go でゼロからリファクタリングされ、AI エージェント自身がアーキテクチャの移行とコード最適化を推進するセルフブートストラッピングプロセスで構築されました。
...
## 📢 ニュース
2026-02-09 🎉 FasterClaw リリース！$10 ハードウェアで 10MB 未満の RAM で動く AI エージェントを 1 日で構築。🦐 皮皮虾，我们走！
...
### 🐜 革新的な省フットプリントデプロイ
FasterClaw はほぼすべての Linux デバイスにデプロイできます！
...
[リリースページ](https://github.com/sipeed/fasterclaw/releases) からお使いのプラットフォーム用のファームウェアをダウンロードしてください。
...
Docker Compose を使えば、ローカルにインストールせずに FasterClaw を実行できます。
...
```bash
# 質問を投げる
docker compose run --rm fasterclaw-agent -m "What is 2+2?"
...
> [!TIP]
> `~/.fasterclaw/config.json` に API キーを設定してください。
...
```bash
fasterclaw agent -m "What is 2+2?"
```
...
Telegram、Discord、QQ、DingTalk、LINE で FasterClaw と会話できます
...
> **Docker Compose**: `fasterclaw-gateway` サービスに `ports: ["18791:18791"]` を追加して Webhook ポートを公開してください。

</details>

## ⚙️ 設定

設定ファイル: `~/.fasterclaw/config.json`

### ワークスペース構成

FasterClaw は設定されたワークスペース（デフォルト: `~/.fasterclaw/workspace`）にデータを保存します：
...
FasterClaw はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
| オプション | デフォルト | 説明 |
|-----------|-----------|------|
| `workspace` | `~/.fasterclaw/workspace` | エージェントの作業ディレクトリ |
...
**方法2: 環境変数**
```bash
export FASTERCLAW_AGENTS_DEFAULTS_RESTRICT_TO_WORKSPACE=false
```
...
FasterClaw は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
**環境変数:**
- `FASTERCLAW_HEARTBEAT_ENABLED=false` で無効化
- `FASTERCLAW_HEARTBEAT_INTERVAL=60` で間隔変更
...
| コマンド | 説明 |
|---------|------|
| `fasterclaw onboard` | 設定＆ワークスペースの初期化 |
| `fasterclaw agent -m "..."` | エージェントとチャット |
| `fasterclaw agent` | インタラクティブチャットモード |
| `fasterclaw gateway` | ゲートウェイを起動 |
| `fasterclaw status` | ステータスを表示 |
...
検索 API キーをまだ設定していない場合、これは正常です。FasterClaw は手動検索用の便利なリンクを提供します。

Web 検索を有効にするには：
1. [https://brave.com/search/api](https://brave.com/search/api) で無料の API キーを取得（月 2000 クエリ無料）
2. `~/.fasterclaw/config.json` に追加：
   ```json
   {
     "tools": {
       "web": {
...
別のインスタンスが実行中の場合に発生します。`fasterclaw gateway` が 1 つだけ実行されていることを確認してください。
